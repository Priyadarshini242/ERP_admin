...
import { AUTH_ENABLED, clearToken, getUser } from "@/lib/auth";
...
function useCurrent() {
  ...
  return { path: pathname, full: qs ? `${pathname}?${qs}` : pathname };
}
...
function Rail({ active, onPick }: { active: NavModule; onPick: (m: NavModule) => void }) {
  return (
...
function Leaf({ item, cur, badge, depth = 1 }: { item: NavItem; cur: { path: string; full: string }; badge?: number; depth?: number }) {
...
function Group({ item, cur, badge, query }: { item: NavItem; cur: { path: string; full: string }; badge?: number; query: string }) {
  ...
  if (!item.children?.length) return <Leaf item={item} cur={cur} badge={item.href?.startsWith("/approvals") ? badge : undefined} />;
  ...
  return (
    <div className="select-none">
      <button
        onClick={() => setOpen((o) => !o)}
        className={clsx(
          "flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-[11px] font-bold uppercase tracking-wider transition",
      ...
      {expanded && (
        <div className="relative ml-[18px] space-y-0.5 border-l border-slate-200 pb-1 pl-1.5 dark:border-ink-700">
          {children.map((c) => (
            <Leaf key={c.label + (c.href ?? "")} item={c} cur={cur} badge={c.href?.startsWith("/approvals") ? badge : undefined} />
          ))}
        </div>
      )}
    </div>
...
function SidebarInner({
  ...
  const active = (picked && MODULES.find((m) => m.key === picked)) || routeModule;
  ...
  const items = useMemo(() => active.items.filter((i) => matches(i, q)), [active, q]);
  const user = getUser();
  ...
  return (
    <>
      ...
      <aside className={clsx("fixed inset-y-0 left-0 z-40 flex transition-transform duration-200 lg:sticky lg:top-0 lg:h-screen lg:translate-x-0", mobileOpen ? "translate-x-0" : "-translate-x-full")}>
        <Rail active={active} onPick={(m) => { setPicked(m.key); setQuery(""); }} />
          ...
          <div className="flex h-full w-[254px] flex-col border-r border-slate-200 bg-white dark:border-ink-700 dark:bg-ink-900">
            ...
            <div className="border-b border-slate-200 px-3.5 pb-3 pt-3.5 dark:border-ink-700">
              ...
              <div className="mt-3 rounded-xl bg-gradient-to-br from-brand-50 to-brand-100/40 px-3 py-2 dark:from-brand-500/10 dark:to-brand-500/5">
                <p className="text-[12.5px] font-bold text-brand-800 dark:text-brand-200">{active.title}</p>
                <p className="truncate text-[10.5px] text-brand-700/70 dark:text-brand-300/70">{active.subtitle}</p>
              </div>
              <div className="relative mt-2.5">
                ...
                <input
                  ...
                  placeholder={`Search ${active.label} menu…`}
            ...
            <nav className="scroll-thin flex-1 space-y-0.5 overflow-y-auto px-2.5 py-2.5">
              {items.map((it) => (
                <Group key={it.label} item={it} cur={cur} badge={actionable} query={q} />
              ))}
